...
import { User, Briefcase, Building2, Code, Zap, Calendar, ListChecks, MessageSquare } from "lucide-react";
...
const GOAL_OPTIONS = [
    ...
    { id: "COLLECT_FEEDBACK", label: "Gather Feedback (Product/Service)", icon: MessageSquare },
...
export default function OnboardingForm(
    ...
    const SelectionCard = ({ option, selected, onSelect }: {
        ...
        return (
            <div
                onClick={onSelect}
                className={cn(
                    "cursor-pointer group border border-border/80 shadow-xs rounded-md p-4 flex items-center gap-4 transition-all",
                    selected && "border-muted bg-primary backdrop-blur-sm text-primary-foreground"
                )}
            >
                <div className="p-3 rounded-xl bg-primary-foreground/10 border border-primary-foreground/20">
    ...
    return (
        <Card className="w-full max-w-2xl border-none outline-none shadow-none bg-transparent p-0">
            <motion.div
                        ...
                        <form onSubmit={formHandleSubmit(handleSubmit)} className="space-y-4">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                                                    {ROLE_OPTIONS.map((role) => (
                                                        <SelectionCard
                                                            ...
                                                            onSelect={() => handleSelect("role", role.id)}
                                                        />
                                                    ))}
                                                </div>
                                            </FormControl>
                            ...
                            <motion.div variants={itemVariants}>
                                <FormField
                                    ...
                                    render={() => (
                                        <FormItem className="gap-2">
                                            <FormLabel className="text-sm font-medium">Your Primary Goal</FormLabel>
                                            <FormControl>
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                            ...
                            <motion.div variants={itemVariants}>
                                <Button
                                    ...
                                    className="w-full py-5 text-base mt-5"
                                >
